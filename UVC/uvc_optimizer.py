...
def uvc_optimizer(optimizer, minimax_model, s_optimizer, r_optimizer, gating_optimizer, dual_optimizer, args, infos, save_budgets, flops_list, z_grad_clip, global_step, gating_interval, gating_grad_list):
    ...
    if minimax_model.model.enable_warmup:
        if minimax_model.block_skip_gating is not None:
            return cur_resource, minimax_model.s.cpu().data.numpy(), minimax_model.r.cpu().data.numpy(), minimax_model.block_skip_gating.cpu().data.numpy(), gating_grad_list
        else:
            return cur_resource, minimax_model.s.cpu().data.numpy(), minimax_model.r.cpu().data.numpy(), minimax_model.block_skip_gating, gating_grad_list
    ...
    if minimax_model.block_skip_gating is not None:
        ...
        return cur_resource, minimax_model.s.cpu().data.numpy(), minimax_model.r.cpu().data.numpy(), minimax_model.block_skip_gating, gating_grad_list, gating_grad_list
...
def build_minimax_model(model, layer_names, uvc_layers, uvc_layers_dict, args, flops_list, vanilla=False):
    ...
    if args.flops_with_mhsa:
        ...
        cost_func = lambda s_,r_,ub_,gating,eps,gumbel_hard : calc_flops(s_, r_, uvc_layers_dict, uvc_layers, args.head_size, s_ub=minimax_model.s_ub, r_ub=minimax_model.r_ub, flops_list=flops_list, gating=gating, full_model_flops=ub_, eps=eps, use_gumbel=args.use_gumbel, gumbel_hard=gumbel_hard)
        resource_ub = float(
            cost_func(
        ...
        resource_fn = lambda s_,r_,gating,eps,gumbel_hard=True: cost_func(s_, r_, resource_ub, gating, eps,gumbel_hard)
