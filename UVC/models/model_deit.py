...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pruning happens on hidden state
        macs += (self.fc1.weight.shape[0] - self.fc1.uvc_s) * x.shape[0]* x.shape[1]* x.shape[2]
        x = self.fc1(x)

        x = self.act(x)
        x = self.drop(x)

        macs += self.fc2.weight.shape[0] * x.shape[0]* x.shape[1]* (x.shape[2] - self.fc2.uvc_s)
        x = self.fc2(x)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        macs += k.transpose(-2, -1).shape[-1] * q.shape[0]* q.shape[1]* q.shape[2]* q.shape[3]

        attn = attn.softmax(dim=-1) # torch.Size([256, 3, 197, 197])
        attn = self.attn_drop(attn)

        # TODO: check the FLOPs here
        x = (attn @ v).transpose(1, 2)
        macs += attn.shape[-1] * v.shape[0]* v.shape[1]* v.shape[2]* v.shape[3]

        indicator_list = compute_indicator(x)
        x = x.reshape(B, N, C)
        x = self.proj(x)

        #TODO: check correctiveness here
        macs += x.shape[0]* x.shape[1]* (x.shape[2] - self.proj.uvc_s)* self.proj.weight.shape[-1]

        x = self.proj_drop(x)
        return x, indicator_list, macs
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        return x[:, 0], l1_list, torch.Tensor(total_macs)

    def forward(self, x):

        x, l1_indicator, total_macs = self.forward_features(x)
        x = self.head(x)
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        return x[:, 0], x[:, 1], l1_list, torch.Tensor(total_macs)

    def forward(self, x):
        x, x_dist, l1_indicator, total_macs = self.forward_features(x)
        x = self.head(x)
